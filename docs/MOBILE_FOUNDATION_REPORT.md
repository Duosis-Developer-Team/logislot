...
- **Önemli düzeltme/ek (aynı gün):** `ingress-nginx-test` controller'ı **hermes-test'i servis ediyor** ve kaldırılan `externalIPs`, Hermes'in node1 80/443 erişim kapısıydı (kaldırılınca yalnızca NodePort 30443 kalmıştı). Kalıcı çözüm: k8s objelerine dokunmadan node1'de **iptables REDIRECT** (443→30443, 80→30880; `/usr/local/sbin/hermes-port-redirect.sh` + `hermes-port-redirect.service` systemd unit ile reboot kalıcı). Doğrulandı: Hermes 443/80/30443 hepsi HTTP 200 ve aynı içerik; node2 kube-ipvs0 temiz; her iki node Ready; LogiSlot login 200. **Ders:** IPVS cluster'da bir Service'e node'un gerçek IP'si `externalIPs` olarak ASLA verilmemeli — aynı erişim node-lokal port yönlendirme veya hostPort ile sağlanmalı. Ayrıca node root parolası yeniden paylaşıldı → rotasyon önerilir.
