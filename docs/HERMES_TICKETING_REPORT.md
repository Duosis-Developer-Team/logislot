...
**`by-source` neden zorunlu:** Hermes'te tekillik kısıtı
`(tenant_id, application_id, source_tenant_row_id, source_ticket_id)`. `source_ticket_id`
kaynak tenant başına tekildir, uygulama başına değil — aynı LogiSlot kurulumundaki
iki müşteri aynı id'yi kullanabilir. Token uygulama sınırını çizer, bu parametre
tenant sınırını. Yani sözleşme metnindeki eksiklik, uygulamadaki hata değil.

### Sapma OLMAYAN: `aggregate_version`

Bir ara "Hermes olay başına `aggregate_version` artırmıyor" diye raporlanmıştı.
**Yanlıştı — hata bizdeydi.** Zarf iki ayrı sayı taşır:

| alan | ne | nasıl kullanılır |
|---|---|---|
| `sequence` | ticket başına **olay** sayacı, her olayda artar | sıralama ve boşluk tespiti |
| `aggregate_version` | ticket'ın **optimistic-lock** sürümü, olay başına artmaz | yazma çağrılarındaki `expected_version` |

Canlı veri (TKT-000003): `created` agg=1 seq=1, `attachment_ready` agg=1 seq=2,
`public_message_added` agg=2 seq=3. Aynı `aggregate_version`ı taşıyan iki olay
**normaldir**; onu sıralamada kullanmak ikinci olayı sessizce düşürüyordu.
Projeksiyon artık `sequence` ile sıralar (`support_ticket_projections.event_sequence`),
`aggregate_version`ı yalnızca saklar.
